...
check_proximity_to_machines()
{
    ...
    while(true)
    {
        ...
        {
            self check_doubletap_proximity();
        }
...
muscle_milk_attack()
{
    ...
    foreach(zombie in zombies)
    {
        ...
        {
            zombie doDamage(zombie.health + 777, zombie.origin, self);
        }
    }
...
{  
    ...
    {  
        ...
        if (extra_damage > 0)  
        {  
            self dodamage(extra_damage, hit_origin, player, player, hit_location, mod);  
        }  
    }  
...
check_doubletap_proximity()
{
    if(isDefined(self.original_doubletap_machine_location))
    {
        if(distance(self.origin, self.original_doubletap_machine_location) <= 70 && !self.machine_is_in_use)
        {
            current_time = GetTime();
            if(current_time - self.last_doubletap_purchase_time >= 200)
            {
                self thread show_punchcola_prompt();
            }
        }
        else if(isDefined(self.current_doubletap_trigger))
        {
            self.current_doubletap_trigger delete();
            self.current_doubletap_trigger = undefined;
        }
...
show_punchcola_prompt()
{
    ...
    if(!isDefined(self.current_doubletap_trigger))
    {
        self.current_doubletap_trigger = create_trigger_radius(
            "Hold ^3[{+activate}]^7 for Punch-a-Cola [Cost: " + self.punchcola_price + "]",
            self.original_doubletap_machine_location + (0, 0, 30)
        );
    }
    
    while(self isTouching(self.current_doubletap_trigger) && 
          self hasPerk("specialty_rof") && 
          !self.machine_is_in_use && 
          !self.is_purchasing)
    {
        if(self useButtonPressed() && self.score >= self.punchcola_price && 
           !self maps\mp\zombies\_zm_laststand::player_is_in_laststand())
        {
            if(isDefined(self.current_doubletap_trigger))
            {
            ...
            if(distance(self.origin, self.original_doubletap_machine_location) <= 70)
            {
                self thread process_punchcola_purchase();
            }
            break;
        }
        else if(self useButtonPressed() && self.score < self.punchcola_price)
        {
            if(distance(self.origin, self.original_doubletap_machine_location) <= 70)
            {
                self playsound("evt_perk_deny");
...
process_punchcola_purchase()
{
    ...
    if(!isDefined(self.original_doubletap_machine_location) || 
       distance(self.origin, self.original_doubletap_machine_location) > 70)
    {
        return;
    }
    ...
    self.machine_is_in_use = true;
    self playsound("zmb_cha_ching");
    self.score -= self.punchcola_price;
